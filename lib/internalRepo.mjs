...
export async function importUploadedDocument(dir, uploadsDir, cwd, document) {
  ...
  const uploadedFile = await persistUploadedOriginal(uploadsDir, cwd, {
    fileName,
    buffer,
    storageFolder: prepared.metadata.storage_folder,
    topicKey: prepared.metadata.topic_key || prepared.title,
  });
  ...
  prepared.source.file_path = uploadedFile.path;
  prepared.metadata.original_file_name = uploadedFile.fileName;
  prepared.metadata.original_file_path = uploadedFile.path;
  prepared.metadata.content_fingerprint = fingerprintContent(prepared.content);
...
async function persistUploadedOriginal(uploadsDir, cwd, file) {
  const normalizedStorageFolder = normalizeStorageFolder(file.storageFolder || "");
  const currentDate = new Date();
  const yearMonth = [
    currentDate.getFullYear(),
    String(currentDate.getMonth() + 1).padStart(2, "0"),
  ].join("-");
  const targetDir = normalizedStorageFolder
    ? path.join(uploadsDir, ...normalizedStorageFolder.split("/"), yearMonth)
    : path.join(uploadsDir, CANONICAL_DIR, yearMonth);
  await mkdir(targetDir, { recursive: true });

  const parsedFile = path.parse(String(file.fileName || "").trim());
  const safeBaseName = sanitizeUploadedFileNamePart(parsedFile.name || "upload");
  const safeExtension = sanitizeUploadedFileExtension(parsedFile.ext || "");
  const timestamp = currentDate.toISOString().replace(/[:.]/g, "-");
  const candidateFileName = `${safeBaseName}${safeExtension}`;
  const fileExists = await stat(path.join(targetDir, candidateFileName))
    .then(() => true)
    .catch(() => false);
  const storedFileName = fileExists
...
function sanitizeUploadedFileNamePart(value) {
...
function sanitizeUploadedFileExtension(value) {
