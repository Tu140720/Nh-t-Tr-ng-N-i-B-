...
let loadedSessionsFile = "";

function resolveSessionsFile(usersFile) {
  return path.resolve(path.dirname(usersFile), "sessions.json");
}

function normalizeSession(record = {}, options = {}) {
  const ttlMs =
    Number.isFinite(Number(options.sessionTtlMs)) && Number(options.sessionTtlMs) > 0
      ? Number(options.sessionTtlMs)
      : DEFAULT_SESSION_TTL_MS;
  const createdAt = Number(record.created_at || Date.now());
  const expiresAt = Number(record.expires_at || createdAt + ttlMs);
  return {
    user_id: String(record.user_id || "").trim(),
    created_at: Number.isFinite(createdAt) ? createdAt : Date.now(),
    expires_at: Number.isFinite(expiresAt) ? expiresAt : Date.now() + ttlMs,
  };
}

function isSessionExpired(session) {
  const expiresAt = Number(session?.expires_at || 0);
  return Number.isFinite(expiresAt) && expiresAt > 0 ? expiresAt <= Date.now() : false;
}
...
async function loadSessions(usersFile, options = {}) {
  const sessionsFile = resolveSessionsFile(usersFile);
  if (loadedSessionsFile === sessionsFile) {
    const pruned = pruneExpiredSessions();
    if (pruned) {
      await saveSessions(usersFile, options);
    }
    return;
  }

  sessions.clear();
  loadedSessionsFile = sessionsFile;
  ...
  try {
    ...
    storedSessions.forEach((item) => {
      const normalizedToken = String(item?.token || "").trim();
      const normalizedSession = normalizeSession(item, options);
      if (normalizedToken && normalizedSession.user_id) {
        sessions.set(normalizedToken, normalizedSession);
      }
    });
  } catch (error) {
...
async function saveSessions(usersFile, options = {}) {
  const sessionsFile = resolveSessionsFile(usersFile);
  loadedSessionsFile = sessionsFile;
  await mkdir(path.dirname(sessionsFile), { recursive: true });
  pruneExpiredSessions();
  const serializedSessions = [...sessions.entries()]
    .map(([token, session]) => ({
      token,
      ...normalizeSession(session, options),
    }))
    .filter((item) => item.token && item.user_id);
  await writeFile(sessionsFile, `${JSON.stringify({ sessions: serializedSessions }, null, 2)}\n`, "utf8");
}

function createSessionRecord(userId, options = {}) {
  const ttlMs =
    Number.isFinite(Number(options.sessionTtlMs)) && Number(options.sessionTtlMs) > 0
      ? Number(options.sessionTtlMs)
      : DEFAULT_SESSION_TTL_MS;
...
export async function resolveCurrentUser(usersFile, request, options = {}) {
  ...
  await loadSessions(usersFile, options);
...
export async function resolveCurrentUserFromToken(usersFile, token, options = {}) {
  ...
  let session = normalizedToken ? sessions.get(normalizedToken) : null;
  if (normalizedToken && session && isSessionExpired(session)) {
...
export async function loginWithPassword(usersFile, username, password, options = {}) {
  ...
  sessions.set(token, createSessionRecord(user.id, options));
  await saveSessions(usersFile, options);
...
export async function verifyLoginOtp(usersFile, challengeToken, code, options = {}) {
  ...
  sessions.set(sessionToken, createSessionRecord(user.id, options));
  await saveSessions(usersFile, options);
...
export async function loginWithUserId(usersFile, userId, options = {}) {
  const users = await loadUsers(usersFile, options);
  const normalizedUserId = String(userId || "").trim();
  const user = users.find((item) => item.id === normalizedUserId);
  if (!user) {
    throw new Error("Khong tim thay tai khoan da lien ket.");
  }
